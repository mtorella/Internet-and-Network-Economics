...
for year in YEARS:
    ...
    # Compute ICT share if both CAPICT_QI and CAP are available
    if "CAPICT_QI" in df_growth_wide.columns and "CAP" in df_growth_wide.columns:
        df_growth_wide["ict_share"] = df_growth_wide["CAPICT_QI"] / df_growth_wide["CAP"].replace(0, float("nan"))
    ...
    # Compute digital intensity: (software + R&D investment) / value added
    year_intangibles_df["dig_intensity"] = (year_intangibles_df["I_Soft_DB"].fillna(0.0) + year_intangibles_df["I_RD"].fillna(0.0)) / year_intangibles_df["VA_CP"].replace(0, float("nan"))

    # Commpute missing values
    n_missing = year_intangibles_df["dig_intensity"].isna().sum()
    if n_missing > 0:
        print(f"{year}: computed digital intensity with {n_missing} missing values")
    ...
    df_digitalisation = df_digitalisation[KEY_COLS + ["ict_share", "dig_intensity"]]

    # Normalise digitalisation metrics to [0,1] range for comparability
    df_digitalisation["ict_share_norm"] = minmax(df_digitalisation["ict_share"])
    df_digitalisation["dig_intensity_norm"] = minmax(df_digitalisation["dig_intensity"])
