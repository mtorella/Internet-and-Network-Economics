...
from tests.Scripts.utils.display import step, print_summary
from tests.Scripts.utils.io import load_csv, ensure_dir, save_csv
from tests.Scripts.utils.transforms import coerce_year_column, select_columns, drop_missing, finalize_year_dtype, pivot_to_wide, filter_year
from tests.Scripts.utils.icio import extract_zblock
...
# Step 3 - Intangible analytical csv
step("STEP 3 - Prepare intangible assets dataset")
...
if not intangible_path.exists():
    ...
    int_df = load_csv(intangible_path)
    if "Unnamed: 0" in int_df.columns:
        int_df = int_df.drop(columns=["Unnamed: 0"])
    int_df = coerce_year_column(int_df)
    int_df = drop_missing(int_df, ["nace_r2_code", "geo_code", "year"])
    int_df = finalize_year_dtype(int_df)

    for year in YEARS:
        year_df = filter_year(int_df, year)
        if year_df.empty:
            print(f"  {year}: no rows found, skipped")
            continue
        save_csv(year_df, PROCESSED_DIR / f"intangibles_analytical_{year}.csv")